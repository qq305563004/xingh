...
public class TreeUtils {
     ...
     * @return
     */
    public static List<PermissionResVo> conversionTree(List<PermissionResVo> permissionVoList) {
        PermissionResVo vo = new PermissionResVo(0, -1);
        permissionVoList.add(vo);
        Collections.sort(permissionVoList);
        createTree(permissionVoList, vo, "id", "parentId", "children");
     ...
     * @return
     */
    public static List<PermissionResVo> generateTree(List<Permission> permissionList) {
        List<PermissionResVo> permissionVo = new ArrayList<>();
        for (Permission permInfo : permissionList) {
            permissionVo.add(new PermissionResVo(permInfo.getId(), permInfo.getParentid(), permInfo.getDescription(), permInfo.getLevel(), permInfo.getMark()));
        }
        PermissionResVo vo = new PermissionResVo(0, -1);
        permissionVo.add(vo);
        createTree(permissionVo, vo, "id", "parentId", "children");
        return vo.getChildren();
    }
     ...
     *            总的权限列表
     */                                    
    public static void selectedLogics(List<PermissionResVo> rolePmsList, List<PermissionResVo> pmsList) {
        boolean funExist;
        for (PermissionResVo rolePmsInfo : rolePmsList) {
            funExist = false;
            for (PermissionResVo pmsInfo : pmsList) {
                if (rolePmsInfo.getId() == pmsInfo.getId()) {
                    pmsInfo.setSelected(true);
                    funExist = true;
                    break;
                }
            }
            if (!funExist) {
                pmsList.add(new PermissionResVo(rolePmsInfo.getId(), rolePmsInfo.getParentId(), rolePmsInfo.getTitle(), rolePmsInfo.getLevel(),
                        rolePmsInfo.getMark(), true));
            }
        }
    }
